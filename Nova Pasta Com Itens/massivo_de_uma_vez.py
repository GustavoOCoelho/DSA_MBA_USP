...
an_exercicio = (2017,2018,2019) #ano do exercicio
bimestre = 6# (6,5,4,3,2,1) #bimestre do an_exercicio nr_periodo
co_tipo_demonstrativo = ('RREO','RREO%20Simplificado') #RREO ou RREO Simplificado, mas simplificado é só pra municipios abaixo de 50k habitantes que fizerem a opção do simplificado
...
resultados = pd.read_csv('~/hello/tabela_resultados_massivo.csv')
...
for ano in an_exercicio:
    ...
    for tipo in co_tipo_demonstrativo:
        ...
        baixar = resultados.query(f'ano{ano} == 0')['ente']
        cidades_baixar = cidades_baixar.merge(baixar,how = 'inner',on='ente')[['cod_ibge','ente','capital','regiao','uf','populacao']].drop_duplicates()
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 'items' in data and data['items']:
                        ...
                        resultados.loc[nome_cidade, f'ano{ano}'] = 1
                        resultados.to_csv('tabela_resultados_massivo.csv')
                        # break
                    else:
                        nome_cidade = cidades.query(f'cod_ibge == {id_ente}')['ente'].values[0]
...
resultados.to_csv('tabela_resultados_massivo_final.csv')