...
baixar2021 = resultado.query('ano2021 == 0')

cidades = cidades.merge(baixar2021,how = 'inner',on='ente')[['cod_ibge','ente','capital','regiao','uf','populacao']].drop_duplicates()
...
for id_ente in cidades['cod_ibge']:
    for bimestre in nr_periodo:
        url = f"https://apidatalake.tesouro.gov.br/ords/siconfi/tt/rreo?an_exercicio=2021&nr_periodo={bimestre}&co_tipo_demonstrativo={co_tipo_demonstrativo}&no_anexo={no_anexo}&co_esfera={co_esfera}&id_ente={id_ente}"
        ...
        # Anexo 13 Demonstrativo das Parcerias Públicos-Privadas; 
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'items' in data and data['items']:
                    ...
                    csv_file_path = f"RREO_anexo_2_{nome_cidade_sem_espacos}_bim{bimestre}_{2021}.csv"
                    ...
                    resultados.loc[nome_cidade, 2021] = 1
                    break
                else:
                    nome_cidade = cidades.query(f'cod_ibge == {id_ente}')['ente'].values[0]
...
resultados.to_csv('tabela_testeresultados_2021.csv')