...
for ano in an_exercicio:
    cidades = pd.read_csv('~/hello/entes.csv')
    resultado1 = pd.read_csv('~/hello/tabela_resultados.csv')
    baixar1 = resultado1.query(f'ano{ano} == 0')
    cidades_baixar1 = cidades.merge(baixar1,how = 'inner',on='ente')[['cod_ibge','ente','capital','regiao','uf','populacao']].drop_duplicates()
    resultado2 = pd.read_csv(f'~/hello/tabela_testeresultados_{ano}.csv')
    baixar2 = resultado2.query('ano == 0')
    print(cidades_baixar1.columns)
    print(baixar2.columns)
    cidades_baixar = cidades_baixar1.merge(baixar2,how = 'inner',on='ente')[['cod_ibge','ente','capital','regiao','uf','populacao']].drop_duplicates()
    for id_ente in cidades_baixar['cod_ibge']:
        for bimestre in nr_periodo:
            url = f"https://apidatalake.tesouro.gov.br/ords/siconfi/tt/rreo?an_exercicio={ano}&nr_periodo={bimestre}&co_tipo_demonstrativo={co_tipo_demonstrativo}&no_anexo={no_anexo}&co_esfera={co_esfera}&id_ente={id_ente}"
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 'items' in data and data['items']:
                        ...
                        csv_file_path = f"RREO_anexo_2_{nome_cidade_sem_espacos}_bim{bimestre}_{ano}.csv"
                        ...
                        resultados.loc[nome_cidade, ano] = 1
                        break
                    else:
                        nome_cidade = cidades.query(f'cod_ibge == {id_ente}')['ente'].values[0]
...
resultados.to_csv('tabela_testeresultados_final.csv')