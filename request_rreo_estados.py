...
cidades = pd.read_csv('~/hello/estados.csv')
...
co_esfera = 'E'#(M)unicipios, (E)stados e DF, (U)nião, (C)onsórcio
...
for id_ente in cidades['cod_ibge']:
    for ano in an_exercicio:
        url = f"https://apidatalake.tesouro.gov.br/ords/siconfi/tt/rreo?an_exercicio={ano}&nr_periodo={nr_periodo}&co_tipo_demonstrativo={co_tipo_demonstrativo}&no_anexo={no_anexo}&co_esfera={co_esfera}&id_ente={id_ente}"
        ...
        if response.status_code == 200:
            ...
            if 'items' in data and data['items']:
                ...
                nome_cidade = cidades.query(f'cod_ibge == {id_ente}')['ente'].values[0]
                print(f"No data found in the response. cidade = {nome_cidade} ano = {ano}")
        else:
            print("Error:", response.status_code)
resultados.to_csv('tabela_resultados.csv')